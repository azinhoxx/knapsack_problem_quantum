...
def buildQUBO(count_of_elements, number_of_slack, weights_array, values_array, lambda_1, lambda_2, max_weight):

    Q = np.zeros((count_of_elements + number_of_slack, count_of_elements + number_of_slack))

    np.fill_diagonal(Q, -values_array)

    diag_values = lambda_2 * weights_array * (weights_array - 2 * max_weight) - lambda_1 * weights_array

    np.fill_diagonal(Q, Q.diagonal() + diag_values)

    off_diag_values = np.triu(np.outer(2 * lambda_2 * weights_array, weights_array), k=1)
...
offset = lambda_2 * max_weight ** 2 + lambda_1 * max_weight
...
for test in range(len(data_values_list)):
    ...
    Q = buildQUBO(count_of_elements, number_of_slack, weights_array, values_array, lambda_1, lambda_2, max_weight)
